...
const handler = async (event, context, callback) => {
    await clientPriv.connect();
    let sum = 0;
    // Query pulls drug IDs from report_dm to be put in shuffle_drugs
    await clientPriv.query(`SELECT DISTINCT drug_id FROM report_dm;`, (err, res) => {
    ...
    await clientPriv.end((err) => {
        console.log("End priv db")
        if (err) throw err;
    });
    ...
    console.log("Connected pub db")

    await clientPub.query('truncate table shuffle_drugs', (error) => {
        if (error) {
            throw error;
        }
    });
